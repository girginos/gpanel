...
**Ne gönderilir:** Yalnızca iki şey — anonim kurulum kimliği (`?id=`) ve panel
sürümü (`?v=`). Kimlik **saf rastgeledir**: hostname, IP veya MAC adresinden
türetilmez, dolayısıyla geri çözülemez ve hangi müşteriye ait olduğu bilinemez.
Amaç yalnızca kaç kurulumun aktif olduğunu sayabilmektir.

**Ne gönderilmez:** Domain adları, hostname, IP adresi, e-posta, müşteri verisi,
veritabanı içeriği, lisans bilgisi, dosya içerikleri. Hiçbiri.

**Sunucu tarafında ne saklanır:** Erişim logu **IP adresi kaydetmez** — yalnızca
zaman damgası, istenen adres (kimlik + sürüm) ve HTTP durum kodu tutulur. Sayım
bu logdaki benzersiz kimlikler üzerinden yapılır.

Kurulum kimliği `/etc/girginospanel/kurulum-kimlik` dosyasında saklanır
(yalnız root okuyabilir). Silerseniz yeni bir kimlik üretilir; bu durumda
kurulumunuz sayımda yeni bir kayıt gibi görünür.

**Kapatmak için** `/etc/girginospanel/env` dosyasına ekleyin ve paneli yeniden
başlatın — bu durumda panel hiçbir dış istek atmaz ve hiçbir şey gönderilmez:
