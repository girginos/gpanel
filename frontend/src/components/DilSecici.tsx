  ...
  return (
    <div className="relative" ref={kokRef}>
      <button
        ...
        className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-dark-600 transition-colors"
      >
      ...
      {acik && (
        <div className="absolute right-0 z-[80] mt-1 w-44 rounded-lg border border-slate-200 dark:border-dark-600 bg-white dark:bg-dark-700 shadow-xl py-1">
          {DILLER.map(d => (
            <button
              key={d.kod}
              type="button"
              ...
              className={`flex w-full items-center gap-2.5 px-3 py-2 text-sm transition-colors ${
                d.kod === aktif.kod
                  ...
                  : 'text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-dark-600'
              }`}
            >
              <Bayrak kod={d.kod} />
              <span>{d.ad}</span>
