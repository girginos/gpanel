...
export default function BackupJobDetayPage() {
  ...
  // Varsayılan: HİÇBİRİ seçili değil — tüm domainler solda "Boş" panelinde başlar,
  ...
  async function geriYukle() {
    if (secili.size === 0) { setHata('Sağdaki “Seçili” listesine en az bir domain taşıyın'); return }
    ...
    } catch (e) { setHata(apiHata(e, 'Geri yükleme başlatılamadı')) }
  ...
  return (
    <div className="px-4 py-4 sm:px-6 sm:py-5">
      <Breadcrumb items={[
        ...
        { etiket: 'Backup Yöneticisi', href: '/backup-yonetimi' },
        { etiket: `İş #${jid}` },
      ]} />
      ...
      {!j ? (
        <div className="text-sm text-slate-400 py-8">Yükleniyor…</div>
      ) : (
        <>
          <div className="rounded-2xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-800/60 p-5 mb-4">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <TurRozet tur={j.tur} />
              <Link to="/backup-yonetimi" className="ml-auto text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200">← İş listesi</Link>
            </div>
            <div className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-xs text-slate-500 dark:text-slate-400">
              <span>{j.tamamlanan}/{j.toplam} tamamlandı{j.durum === 'calisiyor' && j.aktif_domain ? ` · şu an: ${j.aktif_domain}` : ''}</span>
              <span>Başlangıç: <span className="font-mono">{j.baslangic}</span></span>
              {j.bitis && <span>Bitiş: <span className="font-mono">{j.bitis}</span></span>}
          ...
          {j.islem === 'geri' ? (
            <div className="rounded-2xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-800/60 overflow-hidden">
              <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-700/60 text-sm font-semibold text-slate-700 dark:text-slate-200">Geri Yükleme Sonuçları</div>
              <div className="divide-y divide-slate-100 dark:divide-slate-700/60">
                ...
                {(!d?.sonuclar || d.sonuclar.length === 0) && <div className="px-4 py-6 text-center text-sm text-slate-400">{j.durum === 'calisiyor' ? 'İşleniyor…' : 'Sonuç yok.'}</div>}
              </div>
            </div>
          ) : (
            <div className="rounded-2xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-800/60 p-4">
              <div className="flex flex-wrap items-center gap-2 mb-3">
                <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-200">Geri Yükleme</h3>
                <span className="text-xs font-semibold text-slate-500 ml-2">mod:</span>
                {([['tam', 'Tam'], ['dosyalar', 'Yalnız Dosyalar'], ['veritabani', 'Yalnız SQL']] as const).map(([v, et]) => (
                ...
                <button onClick={geriYukle} disabled={gonderiliyor || secili.size === 0}
                  ...
                  {gonderiliyor ? 'Başlatılıyor…' : `Seçilenleri Geri Yükle (${secili.size})`}
                </button>
              </div>
              <TransferListe items={kalemler} secili={secili} onDegis={setSecili} />
              <p className="text-[11px] text-slate-400 mt-2">Domainleri soldan sağa taşıyın (tıklayın ya da başlıktaki kutuyla tümünü taşıyın). Yalnız <b>Seçili</b> listesindekiler geri yüklenir. Dosya/DB bazlı hassas geri yükleme için “detaylı”.</p>
            </div>
          )}
        </>
      )}
...
function TransferListe({ items, secili, onDegis }: {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      <Sutun baslik="Boş" adet={bos.length} items={bosF} ara={araBos} setAra={setAraBos}
        yon="sag" onTasi={id => tasi(id, true)} onHepsi={hepsiSaga} bosMetin="Nesne yok" />
      <Sutun baslik="Seçili" adet={sec.length} items={secF} ara={araSec} setAra={setAraSec}
        yon="sol" onTasi={id => tasi(id, false)} onHepsi={hepsiSola} bosMetin="Seçilen hiçbir nesne yok" />
    </div>
  )
}
    ...
    <div className="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden flex flex-col">
      ...
      <div className="px-2 py-2 border-b border-slate-100 dark:border-slate-700/60 flex items-center gap-2">
        <input type="checkbox" checked={false} onChange={onHepsi} disabled={items.length === 0}
          title={yon === 'sag' ? 'Görünenlerin tümünü seç' : 'Görünenlerin tümünü kaldır'}
      ...
      <div className="max-h-72 min-h-[8rem] overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800 bg-white dark:bg-slate-800/40">
        ...
        {items.map(k => (
          <button key={k.domain_id} onClick={() => onTasi(k.domain_id)}
            ...
            <Link to={`/abonelikler/${k.domain_id}/yedekler`} onClick={e => e.stopPropagation()} className="text-[11px] text-brand-600 dark:text-brand-400 hover:underline shrink-0">detaylı</Link>
