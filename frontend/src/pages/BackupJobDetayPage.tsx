        ...
        <>
          <div className="rounded-2xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-800/60 p-5 mb-4">
            ...
            <div className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-xs text-slate-500 dark:text-slate-400">
              <span>{j.tamamlanan}/{j.toplam} {cevir("tamamlandı")}{j.durum === 'calisiyor' && j.aktif_domain ? cevirT(cevir(" · şu an: {0}"), j.aktif_domain) : ''}</span>
              ...
              {j.boyut_b > 0 && <span>{cevir("Boyut:")} <span className="font-mono">{fmtByte(j.boyut_b)}</span></span>}
              {j.basari > 0 && <span className="text-emerald-600 dark:text-emerald-400">✓ {j.basari} {cevir("başarılı")}</span>}
              {j.hata > 0 && <span className="text-red-600 dark:text-red-400">✕ {j.hata} {cevir("hata")}</span>}
              {j.baslatan && <span>{cevir("Başlatan:")} {j.baslatan}</span>}
            </div>
              ...
              <div className="mt-3">
                <div className="flex items-center justify-between text-xs text-amber-600 dark:text-amber-400 mb-1">
                  <span className="inline-flex items-center gap-1.5">
                    ...
                    {j.islem === 'geri' ? cevir("Geri yükleniyor") : cevir("Yedekleniyor")}…{j.aktif_domain ? ` · ${j.aktif_domain}` : ''}
                  </span>
          ...
          {j.islem === 'geri' ? (
            <div className="rounded-2xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-800/60 overflow-hidden">
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-700/60">
                ...
                {(!d?.sonuclar || d.sonuclar.length === 0) && <div className="px-4 py-6 text-center text-sm text-slate-400">{j.durum === 'calisiyor' ? cevir("İşleniyor…") : cevir("Sonuç yok.")}</div>}
              </div>
            </div>
          ) : (
            <div className="rounded-2xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-800/60 p-4">
              <div className="flex flex-wrap items-center gap-2 mb-3">
                ...
                {(mod === 'tam' || mod === 'dosyalar') && (
                  <label className="flex items-center gap-1 text-[11px] text-amber-700 dark:text-amber-300 ml-1">
                    <input type="checkbox" checked={temiz} onChange={e => setTemiz(e.target.checked)} /> {cevir("temiz (ezerek)")}
                  </label>
                )}
              ...
              <p className="text-[11px] text-slate-400 mt-2">{cevir("Domainleri soldan sağa taşıyın (tıklayın ya da başlıktaki kutuyla tümünü taşıyın). Yalnız")} <b>{cevir("Seçili")}</b> {cevir("listesindekiler geri yüklenir. Dosya/DB bazlı hassas geri yükleme için “detaylı”.")}</p>
            </div>
          )}
        </>
      )}
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      <Sutun baslik={cevir("Boş")} adet={bos.length} items={bosF} ara={araBos} setAra={setAraBos}
        yon="sag" onTasi={id => tasi(id, true)} onHepsi={hepsiSaga} bosMetin={cevir("Nesne yok")} />
