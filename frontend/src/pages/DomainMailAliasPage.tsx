...
export default function DomainMailAliasPage() {
  ...
  function mailDomainYukle(d: Domain) {
    ...
    api.get<{ domainler: MailDomain[] }>('/eklenti/mail/domainler')
      .then(r => { if (_n !== mailDomainYukleNesli.current) return; setMailDomain((r.data.domainler || []).find(x => x.ad.toLowerCase() === d.alan_adi.toLowerCase()) || null) })
  ...
  function yukle(did: number) {
    ...
    api.get<{ aliaslar: Alias[] }>(`/eklenti/mail/aliaslar?domain=${did}`)
      .then(r => { if (_n !== yukleNesli.current) return; setAliaslar(r.data.aliaslar || []) })
      .catch(e => { if (_n !== yukleNesli.current) return; const m = apiHata(e, cevir("Takma adlar alınamadı")); setHata(m); toast.hata(cevir("İşlem başarısız"), m); setAliaslar([]) })
  }
  useEffect(() => { if (mailDomain) yukle(mailDomain.id); return () => { yukleNesli.current++ } }, [mailDomain])
  ...
  async function ekle(e: React.FormEvent) {
    ...
    try {
      ...
      const m = cevirT(cevir("✓ {0} → {1} eklendi."), src, hedef)
      setBildirim(m)
      toast.basari(cevir("Kaydedildi"), m)
      setKaynak(''); setHedef(''); setCatchall(false)
      yukle(mailDomain.id)
    } catch (er) { const m = apiHata(er, cevir("Takma ad eklenemedi")); setHata(m); toast.hata(cevir("İşlem başarısız"), m) }
  ...
  async function sil(a: Alias) {
    ...
    try {
      ...
      toast.basari(cevir("✓ Yönlendirme silindi."))
      if (mailDomain) yukle(mailDomain.id)
    } catch (e) { const m = apiHata(e, cevir("Silinemedi")); setHata(m); toast.hata(cevir("İşlem başarısız"), m) }
    finally { setIsleniyor(false) }
  }
  ...
  return (
    <div className="px-4 py-4 sm:px-6 sm:py-5 max-w-7xl mx-auto">
      ...
      {mailDomain === null && (
        <div className="bg-white dark:bg-dark-700 border border-slate-200 dark:border-dark-600 rounded-lg p-8 text-center text-sm text-slate-500 dark:text-slate-400">
            ...
            {aliaslar && aliaslar.length > 0 && (
              <ul className="divide-y divide-slate-100 dark:divide-dark-600/70">
                {aliaslar.map(a => {
                  ...
                  return (
                    <li key={a.id} className="px-5 py-3.5 flex items-center gap-3 flex-wrap hover:bg-slate-50/70 dark:hover:bg-dark-600/20 transition-colors">
