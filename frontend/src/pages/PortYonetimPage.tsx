  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6 md:px-6 md:py-8">
      ...
      <div className="mt-4 mb-6">
        ...
        <div className="mt-3 rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-200">
      ...
      {yukleniyor ? (
        ...
        <>
          ...
          <div className="mx-auto max-w-2xl">
            {/* Dış port */}
            <form onSubmit={disUygula} className="rounded-lg border border-slate-200 bg-white p-5 dark:border-dark-600 dark:bg-dark-800">
              ...
              <div className="mb-3 flex items-center gap-3">
                ...
                <span className="rounded-md bg-slate-100 px-2 py-1 font-mono text-sm dark:bg-dark-700">:{durum.dis_port}</span>
              </div>
              ...
              <input type="number" value={yeniDis} onChange={(e) => setYeniDis(e.target.value)}
                ...
                className="mb-4 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 font-mono text-sm dark:border-dark-600 dark:bg-dark-900" />
              <Button type="submit" color="error" disabled={!!gonderiliyor || !!aktifCalisiyor || !yeniDis}
          ...
          {yasakli && (
            <details className="mt-6 rounded-lg border border-slate-200 bg-white p-4 text-sm dark:border-dark-600 dark:bg-dark-800">
          ...
          {/* Geçmiş */}
          {gecmis.length > 0 && (
            <div className="mt-6 overflow-hidden rounded-lg border border-slate-200 dark:border-dark-600">
              <div className="border-b border-slate-200 bg-slate-50 px-4 py-2 text-sm font-medium dark:border-dark-600 dark:bg-dark-800">{cevir("Geçmiş (son 20)")}</div>
              <div className="overflow-x-auto">
                <table className="min-w-[640px] w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-dark-600">
                    {gecmis.map((g) => (
                      <tr key={g.id} className="bg-white dark:bg-dark-900">
                        ...
                        <td className="px-3 py-2">
                          {g.basarili ? (
                            <Badge color="success" variant="soft" className="px-2 py-0.5 text-[11px]">{cevir("başarılı")}</Badge>
                          ) : g.rollback ? (
                            <Badge color="warning" variant="soft" className="px-2 py-0.5 text-[11px]">{cevir("geri alındı")}</Badge>
                          ) : (
                            <Badge color="error" variant="soft" className="px-2 py-0.5 text-[11px]">{cevir("başarısız")}</Badge>
                          )}
                        </td>
